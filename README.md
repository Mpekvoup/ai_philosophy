...
- 🔊 **Голосовое озвучивание ответов** через Microsoft Edge-TTS (бесплатно, высокое качество)
...
Google Gemini Cloud API → Философский ответ (ТЕКСТ)
    ↓
Django + Edge-TTS → Генерация аудио из текста (MP3)
    ↓
...
- Убедитесь, что установлен пакет edge-tts: `pip install edge-tts`
